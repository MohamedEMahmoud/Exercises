...
router.patch('/api/exercise', upload.fields([{ name: "media" }]), requireAuth, async (req: Request, res: Response) => {
    ...
    if (exercise.trainee && exercise.trainee !== currentUser!.id) {
        throw new BadRequestError("Something went wrong");
    }

    if (exercise.coach && exercise.coach !== currentUser!.id) {
        throw new BadRequestError("Something went wrong");
    }
